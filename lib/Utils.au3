...
Func CountSlots($fromBag = 1, $toBag = $BAGS_COUNT)
	...
	;~ If bag is missing it just won't count (Slots = 0)
...
Func WaitAndFightEnemiesInArea($options = Null)
	If IsPlayerAndPartyWiped() Then Return $FAIL

	If $options = Null Then $options = CloneDictMap($Default_MoveAggroAndKill_Options)
	Local $fightFunction = ($options <> Null And $options.Item('fightFunction') <> Null) ? $options.Item('fightFunction') : KillFoesInArea
	Local $fightRange = ($options <> Null And $options.Item('fightRange') <> Null) ? $options.Item('fightRange') : $RANGE_EARSHOT * 1.5
	Local $fightDuration = ($options <> Null And $options.Item('fightDuration') <> Null) ? $options.Item('fightDuration') : 60000
...
Func FlagMoveAggroAndKill($x, $y, $log = '', $options = Null)
	If $options = Null Then $options = CloneDictMap($Default_MoveAggroAndKill_Options)
	$options.Item('flagHeroesOnFight') = True
...
Func KillFoesInArea($options = Null)
	If IsPlayerAndPartyWiped() Then Return $FAIL
