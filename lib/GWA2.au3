...
; Each gameClient will be a 4-elements array: [0] = PID, [1] = handle (or 0 if invalidated), [2] = window, [3] = character name
...
Func IsItemEquippedInWeaponSlot($itemModelID, $weaponSlot)
	...
	;~ Bits from 1 to 4 say if item is equipped in weapon slot 1 to 4 respectively. If item is unequipped then value is 0. If same item is equipped in all 4 slots then value is 15 = 1+2+4+8 = 2^0+2^1+2^2+2^3
...
Func GetAgentArray($type = 0)
	...
	Local $returnArray[$count] ;~ dynamic 1D array of agents, indexed from 0
	If $count > 0 Then
		For $i = 0 To $count - 1
