...
; BotsHub Author: caustic-kronos (aka Kronos, Night, Svarog)
...
Global Const $WarSupplyKeiranInformations = 'For best results, have :' & @CRLF _
	...
	& 'This bot is useful for farming War Supplies, festival items, platinum and Ebon Vanguard reputation' & @CRLF _
...
Func GetKeiranBow()
	...
	Local $bowDialogID = 0x0000008A ;~ hexadecimal code of dialog id to receive keiran's bow
...
Func EnterAuspiciousBeginningsQuest()
	...
	Local $questDialogID = 0x0000063E ;~ hexadecimal code of dialog id to start Auspicious Beginnings quest
...
Func WarSupplyFarmFight($options)
	Info('Fighting')
	If(IsPLayerDead()) Then Return $FAIL
	If GetMapID() <> $AuspiciousBeginningsMapID Then Return $FAIL
	If TimerDiff($WarSupplyFarmTimer) > $MAX_WAR_SUPPLY_FARM_DURATION Then Return $FAIL
	...
	While IsPlayerAlive()
		...
		If IsRecharged($KeiranSniperShot) And IsPlayerAlive() Then
			For $i = 0 To UBound($foes) - 1
				If GetHasHex($foes[$i]) And Not GetIsDead($foes[$i]) And DllStructGetData($foes[$i], 'ID') <> 0 Then
					UseSkillEx($KeiranSniperShot, $foes[$i])
					Sleep(800)
		...
		If IsRecharged($KeiranTerminalVelocity) And IsPlayerAlive() Then
			For $i = 0 To UBound($foes) - 1
				If GetIsCasting($foes[$i]) And Not GetIsDead($foes[$i]) And DllStructGetData($foes[$i], 'ID') <> 0 Then
					Switch DllStructGetData($foes[$i], "Skill")
						;~ if foe is casting dangerous AoE skill on player then try to interrupt it and evade AoE location
						Case $MeteorShower, $FireStorm, $RayOfJudgement, $UnsteadyGround, $SandStorm, $SavannahHeat
							UseSkillEx($KeiranTerminalVelocity, $foes[$i]) ;~ attempt to interrupt dangerous AoE skill
							;~ attempt to evade dangerous AoE skill effect just in case interrupt was too late or unsuccessful
							EvadeAoESkillArea()
							ContinueLoop
						Case $HealingSignet, $ResurrectionSignet, $Empathy, $AnimateBoneMinions, $Vengeance, $TrollUnguent, $FleshOfMyFlesh, $AnimateFleshGolem, $ResurrectionChant, $RenewLife, $SignetOfReturn
							UseSkillEx($KeiranTerminalVelocity, $foes[$i]) ;~ attempt to interrupt skill
							ContinueLoop
					EndSwitch
				EndIf
			Next
		...
		For $i = 0 To UBound($foes) - 1
			If BitAND(DllStructGetData($foes[$i], 'TypeMap'), 0x1) == 1 Then $isFoeAttacking = True ;~ first bit in TypeMap corresponds to attack stance
			If GetDistance($Miku, $foes[$i]) < $RANGE_EARSHOT Then $isFoeInRangeOfMiku = True
			;If GetTarget($foes[$i]) == $AgentID_Player Then $isFoeAttackingPlayer = True ;~ unfortunately GetTarget() always returns 0, so can't be used here
			;If GetTarget($foes[$i]) == $AgentID_Miku Then $isFoeAttackingMiku = True ;~ unfortunately GetTarget() always returns 0, so can't be used here
		Next
